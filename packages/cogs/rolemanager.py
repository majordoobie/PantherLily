...
class Role_Manager(commands.Cog):
    ...
    async def roles_find(self, ctx, *, cmd):
        ...
        if primary:
            ...
            await self.bot.embed_print(ctx, title=f'**__MEMBERS WITH {primary_role.name} and {secondary_role.name}__**', description=output)
        ...
        # Send without the with statement
            ...
            for member in role.members:
                output += f"` 00 ` ` {member.name:<25.25} `\n"
            await self.bot.embed_print(ctx, title=f'**__MEMBERS WITH {role.name}__**', description=output)
